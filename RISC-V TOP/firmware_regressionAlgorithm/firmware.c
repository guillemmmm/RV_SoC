...
void calcTAU(volatile uint32_t* regs, uint8_t nRegs, uint32_t* tauRAW){
	...
	uint16_t dada;
	uint32_t y;
	for (nbins=0;nbins<nRegs;nbins++){
		dada = *(regs+nbins);
		if(dada){
			y = log2n((uint32_t)dada);
			sumxsumy += y;
			sumxy += y*nbins;
		} else{ //hem arribat al maxim (primer 0)
			nbins++;
			break;
		}
	}
	uint32_t sumx = nbins * (nbins-1);
	sumx >>= 1;
    sumxsumy *= sumx;
	uint32_t Nsumx2 = sumx * (-1+(nbins<<1));
	Nsumx2 = Nsumx2 / 3;
	Nsumx2 *= nbins;
	
	Nsumx2 = (Nsumx2 - sumx*sumx);
	Nsumx2 <<= (logF+resTAU);
	sumxsumy = sumxsumy - ( nbins * sumxy );//sumx * sumy

	if(sumxsumy){
		*tauRAW = Nsumx2 / sumxsumy;
	} else{
		//error divisio
		*tauRAW = 0xFFFFFFFF; //infinit
	}
}
